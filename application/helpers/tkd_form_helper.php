...
Class Tkd_form_helper
{
	...
	{
        $class = (array_filled($p['attributes']>0))?$p['attributes']['class'] : '';
		return '<input class=" form-control '.$class .'" id="'.$p['id'].'" '.$p['additional'].' name="' . $p['field_name'] .'" type="'.$p['field']['type'].'" value="'.htmlspecialchars($p['field_data']).'"/>';
	}
    ...
    {
        $value = ($p['field_data']==null)?$p['field']['placeholder']:$p['field_data'];
        return '<input class=" form-control '.$p['attributes']['class'] .'" id="'.$p['id'].'" '.$p['additional'].' name="' . $p['field_name'] .'" type="'.$p['field']['type'].'" value="'.$value.'"/>';
    }
    ...
    public function gen_dropdown_readonly($p='')
    {
        ...
        $field_html .= $option_list . "</select>";
        $field_html .= "<script>$(document).ready(function(){
        $( window ).on( 'load', function() {
        console.log('asd');
        $('#s2id_" . $p['table'] . "-".$p['field']['name']."').attr('style', 'pointer-events: none;');
        });
        });</script>";
        return $field_html;
...
<?=humanize($key).': '.$value?>
 ...
 //              </div>
    ...
    public function gen_select2($p='')
    {
        ...
        $is_ajax = 0;
        $is_select_id = "-1";
        if(isset($p["attributes"]["class"])){
            $is_ajax = true;
            $is_select_id = "'" . $p['attributes']['id'] . "'";
        }

        $field_html .= '<script>

        $(document).ready(function () {
        var js_ajax = ' . $is_ajax . ',select_id = ' . $is_select_id .';
            ...
            $("#'. $id .'").select2({
                ...
                if(js_ajax){
                    ...
                    // Get post url
                    var url = ' . "'" .g('base_url') . "'" .' + "product/get-size";
                    // Submit action
                    jQuery.ajax({
                        url: url,
                        type: "POST",
    ...
    public function gen_select2_readonly($p='')
    {
        $this->_CI->register_plugins(array("select2"));
        $id = "select2_cus_".$p['field_key'];
        $field_html = '<input type="hidden" name="'.$p['attributes']['field_name'].'" class="bigdrop form-control" id="'.$id.'" style="width:' . $p['attributes']['width'] . '"/>';
        ...
        $( window ).on( 'load', function() {
                ...
                $('#s2id_" . $p['table'] . "-".$p['field']['name']."').attr('style', 'pointer-events: none;');
                $('#s2id_" . $id . "').attr('style', 'pointer-events: none !important;');
            });
        });</script>";
