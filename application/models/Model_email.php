...
class Model_email extends MY_Model {
    ...
    public function send_welcome_email($signup)
    {
        ...
        $config = array(
            'protocol'  => 'smtp',
            'smtp_host' => 'mail.divineconsignsalesil.com',
            'smtp_port' => 465,
            'smtp_user' => 'info@divineconsignsalesil.com',
            'smtp_pass' => '@Admin!23#',
            'mailtype'  => 'html',
            'charset'   => 'utf-8'
        );
    ...
    public function send_user_active_email($signup,$inserted_id)
    {
        //debug($signup,1);
        $this->load->library('email');
         //SMTP & mail configuration
        ...
        // debug($signup);
        // debug($inserted_id);
        $db_to = $signup['signup_email'] ;
        //debug($db_to,1);
        // $db_to = 'johndavid78663@gmail.com' ;
        //$send_from = g('db.admin.support_email');
        $send_from = 'divineconsignofillinois@gmail.com';
        $cc = 'info@divineconsignsalesil.com';
        $name = g('site_name');
   

        $send_to = $db_to;
        $title = $name .' – New Account';
        $subject = "Divine Consign - New Account";
...
//Prsonal info email function
    public function personal_info_update($signup_data)
    {
        // debug($signup_data);
        $message = '';
        $param['personal_data'] =$signup_data;
        // debug($param['personal_data'],1);
        // $message .= 'Hi ' . ucfirst($signup_data['signup_firstname'] . ' ' . $signup_data['signup_lastname']) . ",<br />";
        $to = $signup_data['signup_email'];
        $from = g("db.admin.support_email") ;
        
        // debug($signup_data['signup_email'],1); 

        $subject = "Divine Consign - Personal info is Updated";
        // debug($subject);

        // $param['comments'] = $message;
        // debug($param['comments'] );
        // Get Logo
        $param['logo'] = $this->model_logo->find_one_active();

        $msg = $this->load->view('_layout/email_template/registration_email_confirmation' , $param , true);
        // debug($msg,1);
        parent::email_structure($to , $from ,$subject, $msg);
    }
    //update email end
    ...
    //sale email function
    public function sale_registration_confirmation($signup_data,$registered_sale)
    {
        //debug($_POST,1);
        // debug($signup_data,1);
        $message = '';
        $param['personal_data'] =$signup_data;
        //debug($sale,1);
        $params['where']['sale_id'] = $_POST['registered_sale']['registered_sale_sale_id'];
        // $params = $_POST['registered_sale']['registered_sale_sale_id'];
        $sale_detail = $this->model_sale->find_one($params);
        // debug ($sale_detail['sale_location'],1);

        $param['sale_location']=$sale_detail['sale_location'];

        $param1['where']['sale_dropoffs_id'] = $_POST['registered_sale']['registered_sale_dropoff_id'];
        $sale_dropoff = $this->model_sale_dropoffs->find_one($param1);

        $param['sale_dropoffs_start_time'] = $sale_dropoff['sale_dropoffs_start_time'];
        $param['sale_dropoffs_end_time'] = $sale_dropoff['sale_dropoffs_end_time'];

        // debug($sale_dropoff['sale_dropoffs_start_time'],1);
        // $param['sale_detail'] =$registered_sale;
        // debug($param['sale_detail'],1);
        // sale_location
        ...
        $subject = "Divine Consign - Sale Registration Confirmation";
        // debug($subject);
        ...
        $msg = $this->load->view('_layout/email_template/sale_registration_confirmation' , $param , true);
        //debug($msg,1);
        parent::email_structure($to , $from ,'Divine Consign - Sale Registration Confirmation', $msg);
    }
    //sale email end

    //volunteer email function
    public function volunteer_registration_confirmation($signup_data,$registered_sale)
    {
        // debug($_POST);
        // debug($signup_data,1);
        $message = '';
        ...
        $params['where']['sale_id'] = $_POST['registered_sale']['registered_sale_sale_id'];
        // $params = $_POST['registered_sale']['registered_sale_sale_id'];
        $param['sale_detail'] = $this->model_sale->find_one($params);
        // debug ($param['sale_detail'],1);

        $param['sale_location']=$sale_detail['sale_location'];

        // $param1['where']['volunteer_shift_id'] = $_POST['registered_sale']['registered_sale_volunteer_id'];
        // $volunteer_detail= $this->model_volunteer_shift->find_one($param1);
        // debug($volunteer_detail,1);

        // $param['volunteer_shift_start_time'] = $volunteer_detail['volunteer_shift_start_time'];
        // $param['volunteer_shift_end_time'] = $volunteer_detail['volunteer_shift_end_time'];
        ...
        $msg = $this->load->view('_layout/email_template/volunteer_email_registration' , $param , true);
        //debug($msg,1);
        parent::email_structure($to , $from ,$subject, $msg);
    }
    //volunteer email end
    ...
    // public function _notification_forgot_password($user_data)
    // {
    //     // debug($user_data);
    //     $message = '';
    //     $param['personal_data'] =$user_data;
    //     // debug($user_data,1);
    //     $message .= 'Dear ' . ucfirst($user_data['signup_firstname'] . ' ' . $user_data['signup_lastname']) . ",<br />";
    ...
    //     $subject = "Divine Consign-Password Reset";
    //     // debug($subject);
    //     // $message .= "Please visit the below link for Password change: <br />";

    //     // $message .= "<a href='". g('base_url') ."user/forgot-password?id=".$user_data['signup_id']."'>CLICK HERE</a>" . "  <br />";
    ...
    public function _notification_forgot_password($signupdata)
    {
        // debug(1,1);  
         //debug($signupdata);
        
        $message = '';
        $param['personal_data'] =$signupdata;
        $message .= 'Dear ' . ucfirst($signupdata['signup_firstname'] . ' ' . $signupdata['signup_lastname']) . ",<br />";
    ...
    public function _notification_forgot_password_smtp($user_data,$token)
    {
        ...
        // $config = array(
        //     'protocol'  => 'smtp',
        //     'smtp_host' => 'mail.designprosca.com',
        //     'smtp_port' => 465,
        //     'smtp_user' => 'dev@designprosca.com',
        //     'smtp_pass' => '@Admin!23#',
        //     'mailtype'  => 'html',
        //     'charset'   => 'utf-8'
        // );
