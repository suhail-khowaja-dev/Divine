	...
	<div class="col-md-8">
		<div class="">
			<div class="pull-right d-flex">
				...
				value="" name="" placeholder="Consignor Number" id="id_number">
				<button type="button" class="btn btn-primary" id="search">Search</button>
			</div>
		</div>
	</div>
	<div class="col-md-4">
		<form class="form-horizontal" id="signup_data" action="<?=g('base_url')?>contact_us/export" method="POST" name="upload_excel" enctype="multipart/form-data">
			<div class="">
				<div class="pull-right">
					...
					<button type="submit" class="btn btn-primary" id="signupdata">Export Sheet</button>
				</div>
			</div>
		</form>
...
<script>
	$(document).ready(function() {
		...
		$("#signupdata2").click(function () {
			
			var url = $("#signup_data").attr("action");
			var data = new FormData(document.getElementById("signup_data"));
			$.ajax({
				url: url,
				type: "POST",
				data: data,
		...
		$('#search').on('click',function(){
			var serach_value = $('#id_number').val();
			var url = '<?php g('base_url')?>'+'detail/'+serach_value;
			// alert(url);
			window.location = url;
		});
	});
